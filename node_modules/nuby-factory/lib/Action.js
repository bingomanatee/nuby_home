...
function Action(params, parent) {
    this.CLASS = 'Action';
    this.validate = {on:false, get:false, put:false, post:false, delete:false};
    this.input = {on:false, get:false, put:false, post:false, delete:false};
    this.process = {on:false, get:false, put:false, post:false, delete:false};
...
_.extend(Action.prototype, {
    ...
    _add:function (method, route, params) {
        if (_DEBUG)  console.log('_add(%s, %s, %s)', method, route, util.inspect(params));
        if (method == 'on') {
            this.config.route = route;
        } else {
            // if any REST methods are set, on is removed.
        ...
        ['validate', 'input', 'process', 'output'].forEach(function (v) {
            if (params[v]) {
                if (!self[v]) throw new Error('trying to add to %s', v);
                self[v][method] = params[v];
            }
        })
    ...
    render:function (cb) {
        ...
        function _render_self() {
            if (_DEBUG) console.log('_render_self() in render: %s ------------ %s', self.name, util.inspect(self));
