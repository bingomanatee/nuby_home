...
function get_path() {
    ...
    file_path.push('/' + this.path_name());
    // console.log('returning path %s', util.inspect(file_path));
    var out = _.map(file_path, proper_path).join('');
    if (_DEBUG) console.log('get_path() returning %s for %s', out, this.name);
    return out;

}

function config_path() {
    return this.get_path() + '/' + this.path_name() + '_config.json';
}
...
module.exports = {
    ...
    name_to_config:_name_to_config,
    path:'',
    mkdir:_mkdir,
    make_dir:make_dir,
    get_path:get_path,
